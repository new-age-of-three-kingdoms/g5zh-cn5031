...
if (!$_POST['gr_id']) { alert('请选择群组ID'); }
if (!$bo_table) { alert('请输入论坛数据表名称'); }
if (!preg_match("/^([A-Za-z0-9_]{1,20})$/", $bo_table)) { alert('论坛版块数据表名称只能使用英文、数字及下划线（20字以内）'); }
if (!$_POST['bo_subject']) { alert('论坛 请输入标题'); }

if ($file = $_POST['bo_include_head']) {
    if (!preg_match("/\.(php|htm['l']?)$/i", $file)) {
        alert('顶部文件只能使用php,html文件');
    }
}

if ($file = $_POST['bo_include_tail']) {
    if (!preg_match("/\.(php|htm['l']?)$/i", $file)) {
        alert('底部文件只能使用php，html文件');
    }
}
