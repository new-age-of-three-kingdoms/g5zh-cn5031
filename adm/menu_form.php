...
<div id="menu_frm" class="new_win">
    ...
    <div class="new_win_desc">
        <label for="me_type">选择对象</label>
        <select name="me_type" id="me_type">
            <option value="">直接输入</option>
            <option value="group">论坛群组</option>
            <option value="board">论坛</option>
...
function add_menu_list(name, link, code)
{
    ...
    list += "<label for=\"me_name_"+ms+"\"  class=\"sound_only\">栏目<strong class=\"sound_only\"> 必选项</strong></label>";
    ...
    list += "</td>";
    list += "<td>";
    list += "<label for=\"me_link_"+ms+"\"  class=\"sound_only\">链接<strong class=\"sound_only\"> 必选项</strong></label>";
    ...
    list += "<label for=\"me_target_"+ms+"\"  class=\"sound_only\">新窗口</label>";
    list += "<select name=\"me_target[]\" id=\"me_target_"+ms+"\">";
    list += "<option value=\"self\">不使用</option>";
    list += "<option value=\"blank\">使用</option>";
    list += "</select>";
    list += "</td>";
    list += "<td class=\"td_numsmall\">";
    list += "<label for=\"me_order_"+ms+"\"  class=\"sound_only\">顺序<strong class=\"sound_only\"> 必选项</strong></label>";
    ...
    list += "<label for=\"me_use_"+ms+"\"  class=\"sound_only\">PC版</label>";
    list += "<select name=\"me_use[]\" id=\"me_use_"+ms+"\">";
    list += "<option value=\"1\">使用</option>";
    list += "<option value=\"0\">不使用</option>";
    list += "</select>";
    list += "</td>";
    list += "<td class=\"td_mngsmall\">";
    list += "<label for=\"me_mobile_use_"+ms+"\"  class=\"sound_only\">触屏版</label>";
    ...
    list += "<td class=\"td_mngsmall\">";
    <?php if($new == 'new') { ?>
    list += "<button type=\"button\" class=\"btn_add_submenu\">添加</button>";
    <?php } ?>
    list += "<button type=\"button\" class=\"btn_del_menu\">删除</button>";
    list += "</td>";
    list += "</tr>";
