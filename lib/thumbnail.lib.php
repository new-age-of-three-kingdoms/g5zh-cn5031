...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if(!is_file($source_file)) // 如果没有原始文件
        return;

    $size = @getimagesize($source_file);
    if($size[2] < 1 || $size[2] > 3) // 仅对gif，jpg，png适用
        return;
    ...
    $thumb_filename = preg_replace("/\.[^\.]+$/i", "", $filename); // 除去扩展名
    ...
    if($is_large) {
        ...
        $dst = imagecreatetruecolor($dst_w, $dst_h);
        $bgcolor = imagecolorallocate($dst, 255, 255, 255); // 背景色
    ...
    chmod($thumb_file, G5_FILE_PERMISSION); // 为了以后的删除操作更改文件类型
...
/*
出处 : http://vikjavev.no/computing/ump.php
New:
...
function is_animated_gif($filename) {
    if(!($fh = @fopen($filename, 'rb')))
        return false;
    $count = 0;
    // 出处 : http://www.php.net/manual/en/function.imagecreatefromgif.php#104473
