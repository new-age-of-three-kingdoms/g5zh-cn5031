...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
			...
			if ($http_res_code < 200 || $http_res_code >= 300) {
				...
				if (($this->response_array == false) || (strlen($this->response_array["LGD_RESPCODE"]) == 0)) {
					...
					if ($this->config['output_UTF8'] == 1)
						...
						$this->response_msg = iconv("utf-8", "gb2312", $this->response_array["LGD_RESPMSG"]);
					
					$this->log("Response Code=[".$this->response_code."], Msg=[".iconv("utf-8", "gb2312", $this->response_array["LGD_RESPMSG"])."], Count=".$this->Response_Count(), LGD_LOG_INFO);
	...
	function Report_TX()
	{
		...
		$response_msg = iconv("utf-8", "gb2312", $response_array["LGD_RESPMSG"]);
	...
	function Response($name, $index=0)
	{
		if ($this->config['output_UTF8'] == 1)
			...
			return (iconv("utf-8", "gb2312", $this->response_array["LGD_RESPONSE"][$index][$name]));
	}
